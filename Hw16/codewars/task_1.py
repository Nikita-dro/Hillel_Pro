...
# Try generating the elements of the resulting list in ascending order, i.e., without resorting to a list reversal
# or prependig the elements to a list.
def climb(n):
    if n == 1:
        return [1]
    else:
        prev_sequence = climb(n // 2)
        current_element = 2 * prev_sequence[-1]
        if n % 2 == 1:
            current_element += 1
        return prev_sequence + [current_element]
